            ...
            try
            {
                ...
                foreach (ReportPeriod period in StaticDataService.QuarterList(from, to).Values.OrderBy(o => o.FromDate))
                {
                    ...
                    result.Add(new Brm.Fluent.Components.Report.QuarterDetail
                    {
                        ...
                        Captured = periodRecords.Where(s => s.CaptureDate != null).Count(),
                        //s => s.CaptureDate >= period.FromDate && s.CaptureDate <= period.ToDate).Count(), //+ erecords.Where(d => d.ProcessDate >= period.FromDate && d.ProcessDate <= period.ToDate).Count(),
