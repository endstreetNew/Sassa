...
namespace Sassa.Brm.Common.Helpers
{
    public static class BulkPrint
    {
        ...
        public static string Body(DcFile file, string GrantType, string RegionName, string LcType, List<RequiredDocsView> docs)
        {
            try
            {
                ...
                //topsection
                sb.Append("<div class=\"topsection\">");
                //Top Row
                sb.Append("<div class=\"row\">");
                //Image
                sb.Append("<div class=\"column3\">");
                ...
                sb.Append("</div>");
                //Middle
                sb.Append("<div class=\"column3 centered\">");
                ...
                sb.Append($"<div class=\"qrcode centered\">");
                sb.Append($"{QrCode}");
                sb.Append("</div>");
                sb.Append("</div>");
                //Right
                sb.Append("<div class=\"column3 right\">");
                sb.Append($"<div>{DateTime.Now.ToShortDateString()}</div>");
                sb.Append("<div>");
                sb.Append($"<label class=\"topchkboxLabel\" >Approved - Main</label><input type=\"checkbox\" {(file.ApplicationStatus.Contains("MAIN") ? "checked" : "")} disabled /><br />");
                sb.Append($"<label class=\"topchkboxLabel\" >Rejected - Archive</label><input type=\"checkbox\" {(file.ApplicationStatus.Contains("ARCHIVE") ? "checked" : "")} disabled /><br />");
                sb.Append($"<label class=\"topchkboxLabel\" >Loose Correspondence</label><input type=\"checkbox\" {(file.ApplicationStatus.Contains("LC") ? "checked" : "")} disabled /><br />");
                sb.Append($"<label class=\"topchkboxLabel\" >Review</label><input type=\"checkbox\" {(file.TransType == 2 ? "checked" : "")} disabled /><br />");
                sb.Append("</div>");
                sb.Append("</div>");
                sb.Append("</div>");
                sb.Append("</div>");
                ...
                //Middle section
                sb.Append("<div class=\"middlesection\">");
                sb.Append("<div class=\"row\">");
                sb.Append("<div class=\"column2\">");
                sb.Append($"<div>{file.FullName}</div>");
                sb.Append($"<div>{RegionName}</div>");
                sb.Append($"<div>Transaction Date: </div>");
                sb.Append($"<div>Date Last Reviewed: </div>");
                sb.Append($"<div>Archive Year: </div>");
                ...
                sb.Append($"<div>{(file.Lastreviewdate == null ? "&nbsp;" : ((DateTime)file.Lastreviewdate).ToString("yyyy/MM/dd"))}</div>");
                sb.Append($"<div>{file.ArchiveYear ?? "&nbsp;"}</div>");
                //endcol
                sb.Append("</div>");
                //endrow div
                sb.Append("</div>");
                ...
                //Right Barcodes
                sb.Append("<div class=\"barcodesection\" >");

                sb.Append($"{IdBarCode}".Replace("</svg>", $"<text x = \"11\" y = \"59\" class=\"small\">{(file.GrantType == "S" ? file.SrdNo + "(Srd)" : file.ApplicantNo + "(Id)")}</text></g></svg>")
                .Replace("viewBox=\"0 0 160 60\"", $"viewBox=\"0 0 60 200\"").Replace("class=\"shape-rendering=&quot;crispEdges&quot;\">", "class=\"barcode shape-rendering=\"crispEdges\"\"><g transform=\"translate(60 0) rotate(90)\">"));

                sb.Append($"{ClmBarCode}".Replace("</svg>", $"<text x = \"11\" y = \"59\" class=\"small\">{file.UnqFileNo}</text></g></svg>")
                .Replace("viewBox=\"0 0 160 60\"", $"viewBox=\"0 0 60 200\"").Replace("class=\"shape-rendering=&quot;crispEdges&quot;\">", "class=\"barcode shape-rendering=\"crispEdges\"\"><g transform=\"translate(60 0) rotate(90)\">"));

                sb.Append($"{BarCode}".Replace("</svg>", $"<text x = \"11\" y = \"59\" class=\"small\">{file.BrmBarcode}</text></g></svg>")
                .Replace("viewBox=\"0 0 160 60\"", $"viewBox=\"0 0 60 200\"").Replace("class=\"shape-rendering=&quot;crispEdges&quot;\">", "class=\"barcode shape-rendering=\"crispEdges\"\"><g transform=\"translate(60 0) rotate(90)\">"));
